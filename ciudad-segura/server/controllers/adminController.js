  ...
  try {
    ...
    const avgResponseTime = resolvedComplaints.length > 0
      ? Math.floor(resolvedComplaints.reduce((sum, c) => {
          ...
          const resolvedAt = new Date(c.fecha_actualizacion || createdAt.getTime() + 2 * 24 * 60 * 60 * 1000);
    ...
    const stats = {
      ...
      pendingComplaints: allComplaints.filter(c => c.estado === 'pendiente').length,
...
exports.updateUser = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Usuario actualizado', data: updatedUser });
  } catch (error) {
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Usuario marcado como eliminado' });
  } catch (error) {
...
// Funciones de organizaciones y autoridades (adaptadas)
...
exports.createOrganization = async (req, res) => {
  try {
    const { nombre, correo_contacto, numero_contacto, ubicacion } = req.body;
    ...
    res.status(500).json({ success: false, message: 'Error al crear organización' });
  }
};
...
exports.getAuthorities = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error al obtener autoridades' });
  }
};
...
exports.getAuthoritiesByOrg = async (req, res) => {
  try {
    ...
    const authorities = await Organization.getAuthoritiesByOrg(parseInt(orgId));
    res.json({ success: true, data: authorities });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error al obtener autoridades de organización' });
  }
};
...
exports.createAuthority = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error al crear autoridad' });
  }
};
...
exports.updateAuthority = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error al actualizar autoridad' });
  }
};
...
exports.deleteAuthority = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM autoridades_detalle WHERE usuario_id = $1 RETURNING *', [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Autoridad no encontrada' });
    }
    res.json({ success: true, message: 'Autoridad eliminada' });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error al eliminar autoridad' });
  }
};