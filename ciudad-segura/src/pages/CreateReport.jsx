...
import ReportMap from '../components/ReportMap';         // Componente
import ReportCategory from '../components/ReportCategory'; // Componente
import ReportEvidence from '../components/ReportEvidence'; // Componente

export default function CreateReport({ user, onBack, onSubmitSuccess, onViewDetails }) {
  ...
  const handleCloseSuccess = () => { setShowSuccess(false); onSubmitSuccess(); };
      ...
      <main className="container" style={{ paddingBottom: '80px' }}>
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          ...
          <form onSubmit={handleSubmit} className="profile-card" style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
            ...
            {/* 1. SECCIÓN: CATEGORÍA (Componente) */}
            <div>
              ...
              <div className="input-group" style={{ marginTop: '20px' }}>
                ...
                <div className="input-wrapper"><textarea rows="3" placeholder="Detalles adicionales..." style={{ ...inputStyle, fontFamily: 'inherit', resize: 'vertical' }}></textarea></div>
              </div>
            </div>

            {/* 2. SECCIÓN: MAPA (Componente) */}
            <div>
              ...
              <div className="input-group"><label style={labelStyle}>Referencia <span style={{fontWeight:400, textTransform:'none', color:'#94a3b8'}}>(Opcional)</span></label><div className="input-wrapper"><input type="text" placeholder="Ej. Frente al parque..." style={inputStyle} /></div></div>
            </div>

            {/* 3. SECCIÓN: EVIDENCIA (Componente) */}
            <div>
            ...
            {user && (
              <div style={{ backgroundColor: '#f8fafc', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0', marginTop: '10px' }}>
                <label style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', cursor: 'pointer' }}>
                  <input type="checkbox" checked={acceptedTerms} onChange={(e) => setAcceptedTerms(e.target.checked)} style={{ width: '20px', height: '20px', accentColor: 'var(--deep-blue)', marginTop: '2px' }} />
                  <span style={{ fontSize: '0.9rem', color: 'var(--deep-blue)', lineHeight: 1.5 }}><strong>Declaración Jurada:</strong> Declaro bajo juramento que la información y evidencia adjunta son verdaderas y actuales. Entiendo que proporcionar información falsa acarrea responsabilidad administrativa y penal.</span>
                </label>
              </div>
            )}
            ...
            <div style={{ paddingTop: '20px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end' }}>
              <Button type="submit" disabled={isSubmitting || (user && !acceptedTerms)} style={{ backgroundColor: (user && !acceptedTerms) ? '#94a3b8' : 'var(--deep-blue)', color: 'white', width: '200px', height: '48px', fontSize: '1rem', cursor: (user && !acceptedTerms) ? 'not-allowed' : 'pointer' }}>{isSubmitting ? 'Enviando...' : 'Enviar Reporte'}</Button>
            </div>
          </form>
        </div>
      </main>
      ...
      {showSuccess && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(14, 42, 59, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000 }}>
          <div style={{ backgroundColor: 'white', borderRadius: '24px', padding: '40px', maxWidth: '450px', width: '90%', textAlign: 'center', boxShadow: '0 20px 25px rgba(0,0,0,0.2)', animation: 'scaleUp 0.3s ease-out' }}>
            <div style={{ width: '80px', height: '80px', margin: '0 auto 24px', backgroundColor: '#dcfce7', color: '#16a34a', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"></polyline></svg></div>
            ...
            <div style={{ backgroundColor: '#f1f5f9', padding: '16px', borderRadius: '12px', marginBottom: '32px', border: '2px dashed var(--border)' }}><span style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 700 }}>Código de Seguimiento</span><span style={{ display: 'block', fontSize: '2rem', fontWeight: 800, color: 'var(--deep-blue)', letterSpacing: '2px', fontFamily: 'monospace' }}>{trackingCode}</span></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}><Button onClick={() => onViewDetails(trackingCode)} style={{ width: '100%', backgroundColor: 'var(--vibrant-blue)', color: 'white' }}>Ver Detalles</Button><Button onClick={handleCloseSuccess} variant="ghost" style={{ width: '100%', color: 'var(--text-muted)', border: '1px solid var(--border)' }}>Volver al Inicio</Button></div>
          </div>
        </div>
      )}
    </div>
